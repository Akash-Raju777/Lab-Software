...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: err?.message || 'Failed to fetch preferences' },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    const { data, error } = await supabase
      .from('user_profiles')
      .update({
        expiry_threshold_days: body.expiryThresholdDays,
        enable_email_alerts: body.enableEmailAlerts,
    ...
    if (error) {
      return NextResponse.json({
        ...
        data: {
          ...
          role: 'ADMIN',
          department: 'Clinical Microbiology',
          expiryThresholdDays: body.expiryThresholdDays,
